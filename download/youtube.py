    ...
    if requested_quality and requested_quality.endswith("p"):
        try:
            ...
            return (
                ...
                f"[height<={height}][ext=mp4]/best[ext=mp4]/best"
            )
        except ValueError:
            pass
    ...
    def fetch_metadata(
        ...
        *,
        ...
        try:
            with YoutubeDL(options) as ydl:
                info = ydl.extract_info(url, download=False)
        except Exception as exc:  # pragma: no cover - mapping handles classification
            error_type, http_status = _map_yt_dlp_error_to_error_type(exc)
            log_with_context(
                logger,
                level=logging.ERROR,
        ...
        *,
        ...
        try:
            with YoutubeDL(options) as ydl:
                ...
                file_path = _resolve_file_path(info, ydl, target_dir)
...
def _resolve_file_path(info: dict[str, Any], ydl: YoutubeDL, target_dir: str) -> str:
    ...
    return os.path.join(target_dir, f"{info.get('id', 'download')}.{info.get('ext', 'dat')}")
